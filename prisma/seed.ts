...
async function seedLocations() {
  const locations = [
    {
      name: "Free Trade Zone (FTZ) Sabang",
      latitude: 0e-30,
      longitude: 0e-30,
    },
    {
    ...
    {
      name: "Free Trade Zone (FTZ) Batam",
      latitude: 0e-30,
      longitude: 0e-30,
    },
    {
  ...
  const locationsOnDatabase = await prisma.location.findMany();
  const newLocations = [];
  ...
  await prisma.location.createMany({ data: newLocations });
}
  ...
  const providers = [
    {
      name: "Pertamina",
      imageUrl:
        "https://opzmuekulpjrmomzojye.supabase.co/storage/v1/object/sign/bbm-files/public/images/pertamina.webp?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJiYm0tZmlsZXMvcHVibGljL2ltYWdlcy9wZXJ0YW1pbmEud2VicCIsImlhdCI6MTcyMjE4Njc1OSwiZXhwIjo0ODc1Nzg2NzU5fQ.GAIYpqWYlUuyuIJhguiatmlmB0ly3okd2Kk97FR9HyA&t=2024-07-28T17%3A12%3A40.556Z",
    ...
    },
    {
      name: "Shell",
      imageUrl:
        "https://opzmuekulpjrmomzojye.supabase.co/storage/v1/object/sign/bbm-files/public/images/shell.webp?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJiYm0tZmlsZXMvcHVibGljL2ltYWdlcy9zaGVsbC53ZWJwIiwiaWF0IjoxNzIyMTg2Nzk0LCJleHAiOjQ4NzU3ODY3OTR9.ADGkuLq5m9euFVu601fkQJkN3LXWT9G1gQATUEzyzG4&t=2024-07-28T17%3A13%3A15.329Z",
      website: "https://www.shell.co.id/",
      isActive: false,
      inactiveReason: "Coming soon!",
    },
  ];

  const providersOnDatabase = await prisma.provider.findMany();
  const newProviders = [];

  for (const provider of providers) {
    if (!providersOnDatabase.find((l) => l.name === provider.name)) {
      newProviders.push(provider);
    }
  }

  await prisma.provider.createMany({ data: newProviders });
}
...
async function seedPages() {
  ...
  await prisma.page.createMany({ data: newPages });
}
